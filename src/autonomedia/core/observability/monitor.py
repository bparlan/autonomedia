        ...
        async with pool.acquire() as conn:
            ...
            for platform in platforms:
                ...
                old_row = await conn.fetchrow(
                    "SELECT status FROM platform_health WHERE platform_name = $1",
                    platform,
                )
                ...
                elif len(history) >= cls.FAILURE_THRESHOLD and all(
                    h["status"] == "failed" for h in history
                ):
                    status = "unhealthy"
                else:
                    status = "healthy"
                ...
                if status != old_status:
                    ...
                    if status == "unhealthy":
                        await notifier.notify(
                            f"⚠️ Platform *{platform}* is UNHEALTHY! 3+ failures detected."
                        )
                    elif status == "healthy":
                        await notifier.notify(
                            f"✅ Platform *{platform}* is back to HEALTHY."
                        )
