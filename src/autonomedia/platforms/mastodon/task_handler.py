    ...
    try:
        ...
        async with BrowserProvider(BROWSER_DATA_DIR, task_id=task_id) as context:
            ...
            login_indicators = await page.get_by_role(
                "link", name=re.compile(r"Log in|Sign in", re.IGNORECASE)
            ).count()
            ...
            if not await compose_textarea.count():
                compose_textarea = page.locator(
                    ".compose-form__autosuggest-wrapper textarea, .compose-form__autosuggest-wrapper div[contenteditable='true']"
                )
