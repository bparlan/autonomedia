    ...
    async def process_prepared_content(self):
        ...
        async with pool.acquire() as conn:
            ...
            rows = await conn.fetch(
                "SELECT * FROM content WHERE status = 'ready_to_post'"
            )
                ...
                try:
                    ...
                    for platform in platforms:
                        ...
                        if history and history["status"] == "published":
                            logger.info(
                                "skip_duplicate", task_id=task_id, platform=platform
                            )
                            continue

                        logger.info(
                            "posting_content", task_id=task_id, platform=platform
                        )

                        try:
                            # We only have mastodon handler right now
                            if platform == "mastodon":
                                result = await publish_mastodon(
                                ...
                                logger.warning(
                                    "unsupported_platform", platform=platform
                                )
                        except Exception as e:
                            logger.error(
                                "platform_posting_failed",
                                task_id=task_id,
                                platform=platform,
                                error=str(e),
                            )
                    ...
                    await conn.execute(
                        "UPDATE content SET status = 'published' WHERE id = $1", task_id
                    )
